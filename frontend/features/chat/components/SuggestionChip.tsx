...
export const SuggestionChip: React.FC<SuggestionChipProps> = ({
  ...
  return (
    ...
    >
      {isLoading ? (
        <Loader2 className="w-3 h-3 text-[#1268D4] dark:text-[#1E88FF] animate-spin shrink-0" />
      ) : (
        <Sparkles className="w-3 h-3 text-[#1268D4] dark:text-[#1E88FF] shrink-0 group-hover:rotate-12 transition-transform" />
      )}
      <span className="truncate max-w-[220px] sm:max-w-[300px]">{label}</span>
      <ArrowUpRight className="w-3 h-3 text-slate-400 dark:text-slate-500 group-hover:text-slate-900 dark:group-hover:text-[#1E88FF] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all shrink-0" />
    </button>
  );
};
